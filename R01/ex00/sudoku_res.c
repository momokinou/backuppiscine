...
/*   Updated: 2017/09/18 19:11:57 by yoginet          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int		backtracking_verif(int **tab, int pos)
{
	...
	if (tab[line][col] != 0)
	{
		if (testline(line, compt, tab) == 0 || testcol(compt, col, tab) == 0
			|| testblock(compt, line, col, tab) == 0)
		{
			ft_putstr("Error_verif\n");
			ft_putchar(line + 48);
			ft_putchar('\n');
			ft_putchar(col + 48);
			ft_putchar('\n');
			ft_putchar(tab[line][col] + 48);
			ft_putchar('\n');
			exit(1);
		}
		return (backtracking_verif(tab, pos + 1));
	}
	return (0);
}
